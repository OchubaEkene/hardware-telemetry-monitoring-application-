...
// ─── Types for structured insights response ───────────────────────────────────
...
// Format structured insights response into readable text for the chat bubble
function formatInsightsResponse(data: InsightsResponse): string {
  const scoreEmoji = data.health_score >= 90 ? "✅" : data.health_score >= 70 ? "⚠️" : "🚨";
  const lines: string[] = [
    `${scoreEmoji} Health Score: ${data.health_score}/100`,
    "",
    data.summary,
  ];
  ...
  if (data.anomalies.length > 0) {
    ...
    data.anomalies.forEach((a) => {
      lines.push(`  • [${a.severity.toUpperCase()}] ${a.metric}: ${a.value} — ${a.detail}`);
    });
  }
  ...
  if (snap) {
    ...
    Object.entries(snap).forEach(([key, s]) => {
      if (s.last !== null) {
        const unit = key.includes("temp") ? "°C" : key.includes("rpm") ? " RPM" : "W";
        lines.push(`  ${key}: last=${s.last}${unit} avg=${s.avg}${unit} trend=${s.trend}`);
      }
    });
  }
...
export default function HardwareAnalyticsScreen() {
  ...
  const [insightsServiceUrl, setInsightsServiceUrl] = useState<string | null>(null);
  const insets = useSafeAreaInsets();

  // Load insights service URL from SecureStore on mount
  useEffect(() => {
    SecureStore.getItemAsync(INSIGHTS_URL_KEY).then((val) => {
      if (val) setInsightsServiceUrl(val);
    });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── Path 1: Claude AI Insights Service ──────────────────────────────────
      const currentInsightsUrl = insightsServiceUrl ?? await SecureStore.getItemAsync(INSIGHTS_URL_KEY);
      if (currentInsightsUrl) {
        try {
          const url = `${currentInsightsUrl}/insights?window_minutes=60&question=${encodeURIComponent(input)}`;
          ...
          if (resp.ok) {
            ...
            responseText = formatInsightsResponse(data);
          }
        } catch (insightsErr) {
          console.warn('Insights service unreachable, falling back to OpenAI:', insightsErr);
        }
      }

      // ── Path 2: OpenAI fallback ──────────────────────────────────────────────
      if (!responseText) {
...
REQUIRED: Use the EXACT values above in your analysis. Do not give generic responses.`
            }
          ],
          maxTokens: 1000,
        });
        ...
        responseText = result.text || `I received your question: "${input}". The AI system is working but returned an empty response.`;
      }
        ...
        <View style={styles.content}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.statusIndicator}>
              <View style={[styles.statusDot, { backgroundColor: insightsServiceUrl ? '#6366f1' : '#10b981' }]} />
              <Text style={styles.statusText}>{insightsServiceUrl ? 'Claude' : 'OpenAI'}</Text>
            </View>
          </View>
              ...
              <View style={styles.loadingContainer}>
                <View style={styles.loadingBubble}>
                  ...
                  <View style={styles.loadingContent}>
                    ...
                    <Text style={styles.loadingText}>{insightsServiceUrl ? 'Querying Claude insights...' : 'Analyzing hardware data...'}</Text>
                  </View>
                </View>
              </View>
            )}
