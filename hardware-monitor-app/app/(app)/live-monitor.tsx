...
export default function LiveMonitorScreen() {
  const { connectionStatus } = useHardware();
  ...
  if (connectionStatus !== 'connected' && telemetryData.length === 0) {
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.loadingContainer}>
          <IconSymbol size={64} name="wifi.slash" color={AppColors.gray400} />
          <Text style={[styles.loadingText, { fontSize: 20, fontWeight: '700', color: '#f8fafc', marginTop: 16 }]}>
            No Hardware Connected
          </Text>
          <Text style={[styles.loadingText, { textAlign: 'center', paddingHorizontal: 32, marginTop: 8 }]}>
            Enter your PC's IP address in the Profile tab to connect the hardware agent.
          </Text>
        </View>
      </SafeAreaView>
    );
